...
class GhostCommand : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if (args == null || args.isEmpty()) {
            ...
            StageGUI(player)
            PositionUtils.getLocationsWithin(
                player.location.add(Vector(15, 0, 15)),
                player.location.add(Vector(-15, 0, -15))
            )
        } else {
            ...
            if (args[0].lowercase().contentEquals("spectate")) {
                ...
                if (player == null || !player.isOnline) {
                    sender.sendMessage(
                        Component.text("This player is not online!").color(TextColor.color(255, 204, 205))
                    )
                    return true
                }
                if (player == sender) {
                    sender.sendMessage(
                        Component.text("You can't spectate yourself!").color(TextColor.color(255, 204, 205))
                    )
                    return true
                }
                GhostCore.getInstance().stageManager.toggleSpectate(
                    sender,
                    GhostCore.getInstance().stageManager.getStages(player)[0]
                )
                return true
            }
        }
